...
const xor = (input, key) => {
    ...
    {
        output[i] = (input[i] ^ key[i % key.byteLength]);
    }
...
const keys = [ // line, key
    ...
    [10, 'no traveller '],
    [13, 'thus conscience ' ],
    [18, 'and lose the name ']
];
// 0 [ 'For who would bear' ] -> google it -> William Shakespeare

const decodeLinesWithKey = (index, key) => {
    const buf_key = strToBuffer(key);
    for (let i = 0; i < lines.length; i ++) {
        const buf_line = strToBuffer(hexToStr(lines[index]));
        const buf_line_next = strToBuffer(hexToStr(lines[i]));
        const xored = xor(buf_line, buf_line_next);
        const xored2 = xor(xored, buf_key);
        const result = bufferToString(xored2);
        console.log(i, [result.substr(0, key.length)]);
    }
}

decodeLinesWithKey(...keys[keys.length-1]);